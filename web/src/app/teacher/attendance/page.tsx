...
export default function TeacherAttendance() {
    ...
    const [excuseEntries, setExcuseEntries] = useState<Record<string, ExcuseEntry>>({});
    const [saved, setSaved] = useState(false);

    const togglePresent = (id: string) => {
        setAttendance(prev => {
            if (prev[id] === "present") return { ...prev, [id]: "absent" };
            setExcuseEntries(e => { const n = { ...e }; delete n[id]; return n; });
    ...
    const setExcused = (id: string) => {
        ...
        setExcuseEntries(prev => ({ ...prev, [id]: { note: prev[id]?.note ?? "", saved: false } }));
        setSaved(false);
    };

    const setAbsent = (id: string) => {
        setAttendance(prev => ({ ...prev, [id]: "absent" }));
        setExcuseEntries(prev => { const n = { ...prev }; delete n[id]; return n; });
        setSaved(false);
    };

    const updateNote = (id: string, note: string) => {
        setExcuseEntries(prev => ({ ...prev, [id]: { note, saved: false } }));
    };

    const saveNote = (id: string) => {
        const entry = excuseEntries[id];
        if (!entry?.note.trim()) return;
        setExcuseEntries(prev => ({ ...prev, [id]: { ...prev[id], saved: true } }));
    };

    // Smart mark-all: if all are present â†’ mark all absent, otherwise mark all present
    const allPresent = useMemo(() => Object.values(attendance).every(v => v === "present"), [attendance]);
    const markAll = () => {
        if (allPresent) {
            setAttendance(Object.fromEntries(students.map(s => [s.id, "absent" as AttendanceStatus])));
        } else {
    ...
    return (
        <div className="page-wrapper">
            ...
            {/* Success Toast */}
            {saved && (
                <div style={{ position: "fixed", top: 20, right: 20, zIndex: 9999, background: "#fff", borderRadius: 14, padding: "1rem 1.5rem", boxShadow: "0 8px 30px rgba(0,0,0,0.12)", border: "1.5px solid var(--success)", display: "flex", alignItems: "center", gap: "0.75rem", animation: "fadeIn 0.3s ease" }}>
            ...
            <div className="card">
                <div className="card-header">
                    <h3 className="card-title" style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                        ...
                        {selectedClass} â€” {new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
                    </h3>
                    <div style={{ fontSize: "0.8rem", color: "var(--gray-500)" }}>Click the checkbox to toggle present/absent</div>
                </div>
                                    ...
                                    <tr key={s.id} style={{ background: isPresent ? "transparent" : isExcused ? "rgba(139,92,246,0.03)" : "rgba(239,68,68,0.03)" }}>
                                        ...
                                        <td>
                                            <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                                                <div style={{ width: 32, height: 32, borderRadius: 8, background: isPresent ? "var(--success-light)" : isExcused ? "var(--purple-light)" : "var(--danger-light)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem", fontWeight: 700, color: isPresent ? "#065f46" : isExcused ? "#5b21b6" : "#991b1b" }}>
                                                ...
                                                <div>
                                                    {excuse?.saved ? (
                                                        <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                                                            ...
                                                            <button onClick={() => setExcuseEntries(prev => ({ ...prev, [s.id]: { ...prev[s.id], saved: false } }))}
                                                        ...
                                                        <div style={{ display: "flex", gap: "0.4rem" }}>
                                                            <input type="text" placeholder="Enter excuse reasonâ€¦" value={excuse?.note ?? ""} onChange={e => updateNote(s.id, e.target.value)}
